---
slug: stryker4s-sbt-speedup
title: 'Improving Stryker4s performance: from 40 minutes to 40 seconds'
...
Stryker4s is your friendly mutation testing framework for Scala. Compared to earlier versions, Stryker4s `0.12` achieves a 100x speed increase 🤯. Are you curious to know how? Keep on reading.
...
- 🔀 Concurrency for even faster performance
...
Doing those checks and starting a new process make a lot of sense for a development workflow, but it's a lot of overhead for mutation testing. This came from the original implementation of the sbt plugin and hasn't changed much in the two years that the plugin has existed. Time for some improvements!
...
The first thing we need to do is figure out what tests your project contains. One of the great things about sbt is its standardization and extensibility as a build tool. Because sbt knows exactly what your project looks like, plugins can retrieve all this information. To find out what tests you have, Stryker4s uses three pieces of information from sbt:

1. `fullClasspath` location of all dependency JARs (including test-dependencies) and `.class` files of your compiled code.
1. `loadedTestFrameworks` the configured test framework. Sbt detects a couple of frameworks by default, but if you use MUnit you might remember having to add `testFrameworks += new TestFramework("munit.Framework")` to your build.sbt before MUnit can run.
1. `testGrouping` contains every test suite sbt discovers in your project.
...
When running your unit tests, Stryker4s activates each mutant separately and runs your unit tests against _only_ the change that mutant creates. It won't activate two mutants at the same time because there's no way to know which of the two mutants gave a result. However, there's no rule that says we can't start multiple Java processes at the same time and run those in parallel. Each process has its own separate memory, threads, and active mutant. This way, if one mutant result in an `OutOfMemoryError` it won't impact the others. Starting multiple testrunner workers also improves performance by making better use of multi-core CPUs.
...
sequenceDiagram
  ...
  Stryker4s->>sbt: compile and retrieve build info
  sbt-->>Stryker4s: return project info
  ...
  testrunner worker->>-Stryker4s: close test worker
`}
/>

As you can see, there's not much overhead left in running your tests. The limiting factor is now how fast your tests are!

## 🔮 What's next?

In this post, we've shown you how Stryker4s uses sbt's test information to reduce overhead and provide massive performance increases. We're very excited for you to try out the increased performance! If you have any feedback or questions, find us on [GitHub](github.com/stryker-mutator/stryker4s) or [Slack](https://join.slack.com/t/stryker-mutator/shared_invite/enQtOTUyMTYyNTg1NDQ0LTU4ODNmZDlmN2I3MmEyMTVhYjZlYmJkOThlNTY3NTM1M2QxYmM5YTM3ODQxYmJjY2YyYzllM2RkMmM1NjNjZjM).
