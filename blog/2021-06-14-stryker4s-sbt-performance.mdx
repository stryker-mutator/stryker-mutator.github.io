...
Stryker4s activates each mutant separately and runs your unit tests against _only_ the change that mutant creates. This is to make sure one mutant won't have an impact on the result of another. However, there's no rule that says we can't start multiple Java processes at the same time and run those in parallel.
